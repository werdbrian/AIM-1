...
namespace AIM.Autoplay
{
    internal class Load
    {
        ...
        public void OnGameLoad(EventArgs args)
        {
            ...
            Notifications.AddNotification(string.Format("AIM {0} Loaded!", Program.Version), 5000);
            Console.WriteLine("Thanks eLem3ntz and weirdbrian4 for updating!");
            Console.WriteLine("Currently still working on a version that improves performance by A LOT.");
        }
        ...
        public static bool UsePorosnax()
        {
            ...
            return trinket != null && trinket.IsReady() && ShouldUsePoroSnaxThisGame &&
                   ObjectManager.Player.Spellbook.CastSpell(SpellSlot.Trinket);
        }
    }
}